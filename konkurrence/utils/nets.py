...
def down_block(ch):
    """Halve H and W with k=3, s=2, p=1."""
    return conv_block(ch, ch, stride=2)

def up_block(in_c, out_c):
    """
    2×2 Transposed Conv (×2 spatial) followed by a conv on concatenated
    feature maps.
    """
...
class UNet16(nn.Module):
    ...
    def __init__(self, n_classes: int, base: int = 64):
        ...
        # ---------------- bottleneck -------------
        self.bottleneck = conv_block(base * 4, base * 8)  # 2×2
    ...
    def forward(self, x: torch.Tensor, t: torch.Tensor, labels: torch.Tensor):
        ...
        signal = torch.cat([img, tt, lab], dim=1) # (B, 3+1+K, 16, 16)
...
def main():
    ...
    model = UNet16(n_classes=num_classes)
